...
fn stdin_name() -> String {
    let pwd = std::env::current_dir().expect("Inkjet: os did not supply working directory");
    String::from(
        pwd.join("stdin")
            .to_str()
            .expect("Inkjet: could not convert stdin path to string"),
    )
}
...
pub fn read_inkfile(inkfile: &str) -> (Result<String, String>, String, bool) {
    ...
    if filename.is_empty() {
        let p = std::env::current_dir().expect("Inkjet: cannot determine current directory");
...
fn read_and_return(file: Result<std::fs::File, std::io::Error>) -> String {
    ...
    file.read_to_string(&mut inkfile_contents)
        .expect("Inkjet: expected file contents");
    inkfile_contents
}
...
pub fn find_inkfile(inkfile_opt: &str) -> (Result<String, String>, String) {
    ...
    if contents.is_err() {
        ...
        let absolute_path = fs::canonicalize(&inkfile_path)
            .expect("Inkjet: canonicalize inkfile path failed")
            .to_str()
            .expect("Inkjet: path contained invalid UTF-8 characters")
            .to_string();
